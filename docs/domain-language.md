...
A related row lights its activity cell with an emphasis background, blank or
not, and bolds its identifying cells (HARNESS and TARGET for Sessions, PATH for
Worktrees, NAME for Branches); the rest of the row keeps its own background,
and Glyph colours, activity, and counts are preserved. The pinned activity
column paints no background of its own, so the lit cell stands alone.
