...
and [ADR 0045](0045-drop-the-up-to-date-rule-where-a-merge-queue-is-unavailable.md):
...
squash merge of a PR whose own `pull_request` run is green. The local review
gate, independent review, and the exact-head verification of the
`pull_request` run remain as recorded here.
