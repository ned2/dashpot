...
| 9 | The 2026-09-13 review's source links were re-pathed to the new module locations but kept `b59032f` line fragments, so `app.py#L1267`, `#L1111` (the file is 971 lines), `repository.py#L350`, and about ten others point at the wrong lines. `scripts/check_docs.py` (since renamed `scripts/maintain_docs.py`) checks only the path half of a fragment. | Minor; pin them to the `b59032f` blob URL or drop the fragments |
