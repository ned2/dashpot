...
[dispatch](dispatch.md), then retry `work start` only after fresh hook evidence
places the session there.
...
`work stop --session <harness>:<session-id>` management command and only when the
...
that it starts a new Agent Session. Do not claim preservation of an active Agent
Run; run `work start` and `work show` in the resulting session.
